...
const props = defineProps<{ id: string; name: string; content: string; useBlobUrl: boolean; useCleanupProtector: boolean }>();
...
const src_prop = computed((old_src_prop?: { srcdoc?: string; src?: string }) => {
  ...
  const content = createSrcContent(props.content, props.useBlobUrl, props.useCleanupProtector);
  if (!props.useBlobUrl) {
